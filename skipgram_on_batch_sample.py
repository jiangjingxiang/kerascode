...
from word2veclayer import WordContextProduct
...
if train_model:
    ...
    for e in range(nb_epoch):
        ...
        for i, seq in enumerate(tokenizer.texts_to_sequences_generator(text_generator())):
            ...
            couples_batch, labels_batch = sequence.skipgrams(seq, max_features, window_size=winsize,
                                                 negative_samples=neg, sampling_table=sampling_table,
                                                 ng_sampling_table=ng_sampling_table)
            if couples_batch:
                # one gradient update per sentence (one sentence = a few 1000s of word couples)
                X = np.array(couples_batch, dtype="int32")
                loss = model.train_on_batch(X, labels_batch)
                losses.append(loss)
                if len(losses) % 100 == 0:
                    print(i)
                    progbar.update(i, values=[("loss", np.mean(losses))])
                    losses = []
                samples_seen += len(labels_batch)
                couples_batch = []
                labels_batch = []
...
def save_embedding(save_dir, file_name):
    print("saving embedding...")
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    fout = open(os.path.join(save_dir, file_name), 'w')
    fout.write("%s %s\n" % (len(word_index), dim_proj))
    for (w, i) in sorted(word_index.items(), lambda x, y: cmp(x[1], y[1])):
        if (not i) or (i < skip_top) or (i >= max_features):
            continue
        fout.write("%s %s\n" % (w,' '.join("%f" % val for val in norm_weights[i])))
    fout.close()
...
for w in words:
    ...
    for r in res:
        print(r)
print ("time consumed:", time.time() - last_time)
t = time.time()-last_time
embed_file = "embedding-dim" + str(dim_proj) + "-win" + str(winsize) + "-neg" + str(neg)
embeding_save_dir = "./"
save_embedding(embeding_save_dir, embed_file)
print ("time consumed:", time.time() - last_time)